...
public class LoggerRateLimiter {
    private Map<String, Integer> logMemo;

    public LoggerRateLimiter() {
        this.logMemo = new HashMap<>();
    }

    public boolean shouldPrintMessage(int timestamp, String message) {

        if (logMemo.containsKey(message)) {
            int logTimestamp = logMemo.get(message);
            if ((timestamp - logTimestamp) < 10) {
                return false;
            }
        }
        logMemo.put(message, timestamp);
        return true;
    }
    /**
